...
{
    /// <summary>
    /// Inspector for <see cref="AnimatorTriggerMarker"/> that offers the trigger names actually declared on the
    /// bound <see cref="Animator"/>, rather than leaving the field a free-form string to mistype.
    /// </summary>
    /// <remarks>
    /// The candidates come from the animator bound to the marker's own track on the director currently previewing
    /// the timeline, so the list is empty until the timeline is open with a director selected and that track bound.
    /// </remarks>
    ...
    public class AnimatorTriggerMarkerDrawer : Editor
    {
        ...
        /// <summary>
        /// Builds the marker inspector: the trigger field, plus — when a bound animator can be resolved — the
        /// animator it will drive and buttons for each trigger parameter that animator declares.
        /// </summary>
        /// <returns>The root element of the inspector.</returns>
